import type { Bottle, Cellar } from '@/lib/api';
...
import { BottleActions } from '@/components/bottle-actions';
import { Wine } from 'lucide-react';

export default async function BottlesPage() {
  const api = serverApi();
  const [bottles, cellars] = await Promise.all([
    withFallback<Bottle[]>(() => api.get('/bottles'), demoBottles),
    withFallback<Cellar[]>(() => api.get('/cellars'), []),
  ]);
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <header className="flex items-end justify-between">
        ...
        <AddBottleDialog cellars={cellars} />
      </header>

      {bottles.length === 0 ? (
        <Card className="flex flex-col items-center gap-3 py-16 text-center">
          <Wine size={32} className="text-neutral-600" />
          <p className="text-sm text-neutral-400">
            Votre cave est vide. Ajoutez votre première bouteille — l’IA remplit
            la fiche pour vous.
          </p>
          <AddBottleDialog cellars={cellars} />
        </Card>
