...
# Le Dockerfile de l'addon compile le frontend lui-même — plus besoin de npm run build.
...
# Sources frontend (compilées lors du docker build par le Dockerfile de l'addon)
for item in src public index.html vite.config.js package.json package-lock.json tailwind.config.js postcss.config.js; do
  rm -rf "$ADDON_DIR/$item"
  [ -e "$REPO_DIR/$item" ] && cp -r "$REPO_DIR/$item" "$ADDON_DIR/$item"
done
...
echo "   (le build du frontend est fait automatiquement, ~3-5 min)"
