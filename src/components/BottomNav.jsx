...
function usePendingTaskCount() {
  ...
  useEffect(() => {
    api.get('/taches').then(tasks => {
      setCount((tasks || []).filter(t => t.statut !== 'termine').length)
    }).catch(() => {})
  }, [refreshTick])
  return count
}
...
export default function BottomNav() {
  ...
  const tabs = [
    ...
    { to: '/taches',    icon: CheckSquare, label: 'Tâches',   badge: pendingCount || null },
            ...
            {({ isActive }) => (
              <>
                ...
                <div className="relative">
                  ...
                  {badge != null && (
                    <span className="absolute -top-1.5 -right-2.5 min-w-[15px] h-[15px] rounded-full bg-red-500 text-white text-[8px] font-bold flex items-center justify-center px-0.5 leading-none">
                      {badge > 99 ? '99+' : badge}
                    </span>
                  )}
                </div>
