...
function ParcelleRows({ parcelle }) {
  ...
  return chargements.map((c, idx) => {
    const isFirst = idx === 0
    const isLast  = idx === chargements.length - 1
    return (
      <tr key={c.id} className="border border-gray-300">
        ...
        <td className="border border-gray-300 px-3 py-1.5">
          <div className="flex items-center justify-between gap-2">
            ...
            {isLast && chargements.length > 1 && (
              <span className="font-bold border-l border-gray-400 pl-2 ml-1">
                {parcelle.poids_total?.toFixed(0)} kg
              </span>
            )}
          </div>
        </td>
