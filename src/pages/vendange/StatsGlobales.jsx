...
// Courbe de Catmull-Rom → chemin SVG lisse
...
function RendementChart({ data }) {
  ...
  const attenduVals = active.map(d => d.rendement_attendu_kgha || 0)
  const maxY = Math.max(...vals, ...attenduVals) * 1.2
  ...
  const midVal = Math.round(maxY / 2 / 1.2)
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full">
      ...
      {/* Objectif dashed */}
      {attenduPath && (
        <path d={attenduPath} fill="none" stroke="#d1d5db"
              strokeWidth="1.5" strokeDasharray="5,3" strokeLinecap="round" />
      )}

      {/* Ligne principale */}
      <path d={linePath} fill="none" stroke="#d97706"
      ...
      {pts.map(p => (
        <g key={p.annee}>
          ...
          <text x={p.x} y={p.y - 9} textAnchor="middle" fontSize="9.5" fill="#92400e" fontWeight="700">
            {fmtK(p.val)}
          </text>
        </g>
      ))}
      ...
      {/* Années */}
      {active.map((d, i) => (
        <text key={d.annee} x={xPos(i)} y={H - 7}
        ...
        return (
          <g key={d.annee}>
            <rect x={x} y={y} width={barW} height={bh} rx="5"
                  fill={isBest ? '#15803d' : 'url(#prod-grad)'} />
            <text x={xCenter(i)} y={y - 5} textAnchor="middle"
                  fontSize="9.5" fill="#14532d" fontWeight={isBest ? '700' : '600'}>
              {fmtK(d.poids_total)}
            </text>
            <text x={xCenter(i)} y={H - 7} textAnchor="middle" fontSize="10" fill="#9ca3af">
              {d.annee}
            </text>
          </g>
        )
...
export default function StatsGlobales() {
  ...
  const avgKgHa  = withRendement.length
    ? Math.round(withRendement.reduce((s, c) => s + c.rendement_kgha, 0) / withRendement.length)
    : null

  // Ordre DESC pour la liste
    ...
    <div className="pb-24">
      ...
      <div className="bg-gradient-to-br from-vigne-900 via-vigne-800 to-vigne-700 px-4 pt-5 pb-8">
        <div className="grid grid-cols-2 gap-3">
          ...
          <HeroCard
            value={caToDisplayHa(surface_totale_ca)}
            label="Surface totale"
          />
          <HeroCard
            value={`${fmtK(totalKg)} kg`}
            label={`en ${campagnes.length} campagne${campagnes.length > 1 ? 's' : ''}`}
          />
          <HeroCard
            value={avgKgHa ? `${avgKgHa.toLocaleString('fr-FR')} kg/ha` : '—'}
            ...
            {campagnes.some(c => c.rendement_attendu_kgha) && (
              <div className="flex items-center gap-1 text-xs text-gray-400">
                <svg width="14" height="6">
                  <line x1="0" y1="3" x2="14" y2="3" stroke="#d1d5db" strokeWidth="1.5" strokeDasharray="4,2" />
                </svg>
                objectif
              </div>
            )}
              ...
              >
                ...
                <div className={`w-12 h-12 rounded-2xl flex flex-col items-center justify-center flex-shrink-0 ${
                  ...
                  <span className={`font-bold text-sm leading-tight ${
                    isBest ? 'text-amber-700' : isClosed ? 'text-gray-500' : 'text-amber-600'
                  }`}>{c.annee}</span>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-baseline gap-1.5">
                    ...
                    {c.rendement_kgha && (
                      <p className="text-xs text-vigne-700 font-medium">
                        · {c.rendement_kgha.toLocaleString('fr-FR')} kg/ha
                      </p>
                    )}
                  </div>
...
function HeroCard({ value, label, accent }) {
  return (
    <div className={`rounded-2xl px-3.5 py-3 ${
      ...
      <p className={`text-xl font-bold leading-snug ${accent ? 'text-amber-300' : 'text-white'}`}>
        {value}
      </p>
